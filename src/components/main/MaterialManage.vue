  ...
  <el-row class="warp">
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
        ...
        <el-row v-else>
          <el-col v-for="(editor,index) in editArr" :span="edit_span" :key="editor.field_desc">
            ...
            <!-- vv此处自定义主表输入类型 -->
            <el-form-item :label="editor.field_desc" v-else-if="editor.field_type=='selectgoods'" :prop="editor.field_name">
              <el-autocomplete class="inline-input" size="small" v-model="editForm[editor.field_name]" :fetch-suggestions="querySearch_sku" placeholder="请输入或搜索货号" @select="handleSelect_sku" ></el-autocomplete>
              <el-button type="success" :loading="button1loading" @click="searchgoodsku" size="small">搜索商品</el-button>
            </el-form-item>
            <el-form-item :label="editor.field_desc" v-else-if="editor.field_type=='selectparam'" :prop="editor.field_name">
              <el-autocomplete class="inline-input" size="small" v-model="editForm[editor.field_name]" placeholder="请输入或选择规格" :fetch-suggestions="querySearch_param" @select="handleSelect_param"></el-autocomplete>
            </el-form-item>
            <el-form-item :label="editor.field_desc" v-else-if="editor.field_type=='sec_measure_rate'" :prop="editor.field_name">
              <el-col :span="6"><el-input type="number" v-model.number="editForm[editor.field_name]" size="small" auto-complete="off" :disabled="editor.couldwrite" :key="index" @blur="form_blur(editForm[editor.field_name],editor.field_name)"></el-input></el-col>
              <el-col :span="1">&nbsp</el-col>
              <el-col :span="17" v-if="editForm.sec_measure_rate && editForm.sec_measure_unit && editForm.measure_unit">（{{editForm.sec_measure_rate}} {{editForm.measure_unit}} = 1 {{editForm.sec_measure_unit}}）</el-col>
            </el-form-item>
            <el-form-item :label="editor.field_desc" v-else-if="editor.field_type=='thi_measure_rate'" :prop="editor.field_name">
              <el-col :span="6"><el-input type="number" v-model.number="editForm[editor.field_name]" size="small" auto-complete="off" :disabled="editor.couldwrite" :key="index" @blur="form_blur(editForm[editor.field_name],editor.field_name)"></el-input></el-col>
              <el-col :span="1">&nbsp</el-col>
              <el-col :span="17" v-if="editForm.thi_measure_rate && editForm.thi_measure_unit && editForm.measure_unit">（{{editForm.thi_measure_rate}} {{editForm.measure_unit}} = 1 {{editForm.thi_measure_unit}}）</el-col>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他按钮 -->
        <el-button type="primary" @click.native="editSubmit" v-if="!refuseChange">保存</el-button>
      </div>
    </el-dialog>
    <!-- 带出表 -->
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["主数据","物料管理"];
  let breadUrls = [""];
  let modelName = "物料";
  let detailModelName = '';
  let key_name = 'material_id'; // 主键名
  let search_filters = ['material_id','material_name','brand_name','goods_sku','class_name','goods_parameter_name']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "normal"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'materialAction.api?getMaterialList'; // 查找获取列表API
  let getDetailAPI = ''; // 获取编辑页信息API
  let saveAPI = 'materialAction.api?saveMaterial'; // 保存API
  let columns = 2;
  let defaultValues = {};
  let enums = {};
  let selectoptions = {};
  let popselect = {
    default_vendors_code:{ // 选择框字段名
      modelname: 'default_vendors_code', // 选择框字段名
      getlistAPI: 'vendorsAction.api?getVendorsList', // 选择框获取列表接口
      key_name:'vendors_id', // 选择框列表主键（默认回带字段）
      search_filter_list: [
        {field_name:'vendors_id',field_desc:'供应商编号'},
        {field_name:'vendors_name',field_desc:'供应商名称'}
      ]
  ...
  let rules = {
    material_name: [
      { required: true, message: '物料名称必填', trigger: 'blur'},
      { min: 1, max: 250, message: '长度不符合要求', trigger: 'blur' }
    ],
    measure_unit: [
      { required: true, message: '基础计量单位必填', trigger: 'blur' },
    ]
  }; // 保存约束
  let detailRules = {}; // 子表保存约束
  /* 模板配置结束 */
  ...
  export default{
    ...
    data(){
      return {
        ...
        /* vv此处自定义数据 */
        button1loading: false, // 搜索货号按钮加载中标记
        goodskus: [], // 货号
        goodsParameterBeans: {}, // 规格
      };
    },
    methods: {
      // 获取列表数据
      ...
      // 搜索货号
      searchgoodsku(){
        if(this.editForm["goods_sku"] != "" && this.editForm["goods_sku"] != undefined){
          this.button1loading = true;
          this.goodskus.length = 0;
          let pros = {goods_state:1, goods_sku:this.editForm["goods_sku"]};
          this.$ajax.postu('materialAction.api?getGoodsList', pros).then((res) => {
            if (res.status == "ok") {
              let datarr = res.data;
              if (datarr.length == 0){
                this.goodskus.length = 0;
                ...
                this.goodsParameterBeans.length = 0;
                // 清空现有规格
                this.editForm.parameter_id = '';
                this.editForm.goods_parameter_name = '';
                var newdata = {};
//                newdata.label = datarr[0].goods_name;
                newdata.value = datarr[0].goods_sku;
                newdata.goods_id = datarr[0].goods_id;
                newdata.class_name = datarr[0].class_name;
                newdata.brand_name = datarr[0].brand_name;
                newdata.goods_name = datarr[0].goods_name;
                this.goodskus.push(newdata);
                // 默认带出
                this.editForm.material_name = datarr[0].goods_name;
                this.editForm.goods_sku = datarr[0].goods_sku;
                this.editForm.goods_id = datarr[0].goods_id;
                this.editForm.class_name = datarr[0].class_name;
                this.editForm.brand_name = datarr[0].brand_name;
                // 规格
                var params = datarr[0].goodsParameterBeans;
                if(params.length > 0){
                  var the_goodsParameterBeans = params[0].goodsParameterBeans;
                  this.goodsParameterBeans[datarr[0].goods_id] = [];
                  for (var j = 0;j < the_goodsParameterBeans.length; ++j){
                    this.goodsParameterBeans[datarr[0].goods_id].push({
                      value:the_goodsParameterBeans[j].parameter_name,
                      p_id:the_goodsParameterBeans[j].parameter_id}
                    );
                  }
                  // 默认带出
                  if(the_goodsParameterBeans.length == 1){
                    this.editForm.parameter_id = the_goodsParameterBeans[0].parameter_id;
                    this.editForm.goods_parameter_name = the_goodsParameterBeans[0].parameter_name;
                  }
                }
              }else{
                this.goodsParameterBeans.length = 0;
                ...
                for(var i=0;i<datarr.length;++i){
                  var newdata = {};
//                  newdata.label = datarr[i].goods_name;
                  newdata.value = datarr[i].goods_sku;
                  newdata.goods_id = datarr[i].goods_id;
                  newdata.class_name = datarr[i].class_name;
                  newdata.brand_name = datarr[i].brand_name;
                  newdata.goods_name = datarr[i].goods_name;
                  this.goodskus.push(newdata);
                  // 规格
                  var params = datarr[i].goodsParameterBeans;
                  this.goodsParameterBeans[datarr[i].goods_id] = [];
                  if(params.length > 0){
                    var the_goodsParameterBeans = params[0].goodsParameterBeans;
                    for (var j = 0;j < the_goodsParameterBeans.length; ++j){
                      this.goodsParameterBeans[datarr[i].goods_id].push({
                        value:the_goodsParameterBeans[j].parameter_name,
                        p_id:the_goodsParameterBeans[j].parameter_id}
                      );
                    }
      ...
      querySearch_sku(queryString, cb) {
        var goodskus = this.goodskus;
        var results = queryString ? goodskus.filter(this.createFilter(queryString)) : goodskus;
        // 调用 callback 返回建议列表的数据
        cb(results);
      },
      querySearch_param(queryString, cb) {
        cb(this.goodsParameterBeans[this.editForm.goods_id]);
      },
      createFilter(queryString) {
        return (goodskus) => {
          let goodskusvalue = goodskus.value.toLowerCase();
          return (goodskusvalue.indexOf(queryString.toLowerCase()) === 0);
        };
      },
      handleSelect_sku(item) {
        this.editForm.material_name = item.goods_name;
        this.editForm.goods_id = item.goods_id;
        this.editForm.class_name = item.class_name;
        this.editForm.brand_name = item.brand_name;
        // 设置规格
        let vd = this.goodsParameterBeans[item.goods_id];
        if(vd.length == 1){
          this.editForm.goods_parameter_name = vd[0].value;
          this.editForm.parameter_id = vd[0].p_id;
        }else{
          this.editForm.goods_parameter_name = '';
          this.editForm.parameter_id = '';
        }
      },
      handleSelect_param(item) {
        this.editForm.parameter_id = item.p_id;
      }
    },
    mounted() {
