...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["主数据","供应商管理"];
  let breadUrls = [""];
  let modelName = "供应商";
  let detailModelName = '联系人';
  let key_name = 'vendors_id'; // 主键名
  let search_filters = ['vendors_id','vendors_code','vendors_name']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'vendorsAction.api?getVendorsList'; // 查找获取列表API
  let getDetailAPI = 'vendorsAction.api?getVendors'; // 获取编辑页子表信息API
  let saveAPI = 'vendorsAction.api?saveVendors'; // 保存API
  let columns = 3;
  let defaultValues = {};
  let enums = {};
  let selectoptions = {};
  ...
  export default{
    ...
    methods: {
      ...
      addNewLine() {
        ...
        // hy add 联系人表设置关联
        newline.relation_type = 'vendors';
        newline.relation_id = this.editidnow;
