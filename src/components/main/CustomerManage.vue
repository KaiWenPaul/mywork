...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["主数据","客户管理"];
  let breadUrls = [""];
  let modelName = "客户";
  let detailModelName = '';
  let key_name = 'customer_id'; // 主键名
  let search_filters = ['customer_name']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "normal"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'customerAction.api?getCustomerList'; // 查找获取列表API
  let getDetailAPI = 'customerAction.api?getCustomer'; // 获取编辑页子表信息API
  let saveAPI = 'customerAction.api?saveCustomer'; // 保存API
  let columns = 3;
  let defaultValues = {pay_period:'30'}; // 默认值,形如state:'0',time:new Date()
