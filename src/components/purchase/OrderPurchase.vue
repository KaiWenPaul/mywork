  ...
  <el-row class="warp">
    ...
    <el-col :span="24" class="warp-main">
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters">
          <div style="width:100%;">
            <el-col v-for="search_filter in search_filter_list" :span="6" :key="search_filter.field_name">
            ...
            <el-col :span="6" v-for="search_enums in search_enums_list" :key="search_enums.field_name">
          ...
          <div style="width:100%;text-align:right;">
            ...
            <el-form-item>
              <el-button class="filter-item" type="primary" icon="document" v-on:click="showExport">导出</el-button>
            </el-form-item>
    ...
    <el-table :data="modelDatas" highlight-current-row v-loading="listLoading" style="width: 100%;" >
      ...
      <el-table-column label="操作" width="190" fixed="right">
        <template slot-scope="scope">
          <!-- vv自定义按钮 -->
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row, 1)" v-if="scope.row.purchase_approval_state == 1 || scope.row.payoff_approval_state == 1">查看</el-button>
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row)" v-if="scope.row.purchase_approval_state != 1 && scope.row.payoff_approval_state != 1">编辑</el-button>
          <el-button type="danger" @click="delBook(scope.$index, scope.row)" size="small" v-if="scope.row.purchase_approval_state != 1 && scope.row.payoff_approval_state != 1">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
        ...
        <el-row v-else>
          <el-col v-for="(editor,index) in editArr" :span="edit_span" :key="editor.field_desc">
            ...
            <!-- vv此处自定义主表输入类型 -->
            <el-form-item :label="editor.field_desc" v-if="editor.field_type=='getorder'" :prop="editor.field_name">
              <el-col :span="19"><el-input v-model="editForm[editor.field_name]" size="small" auto-complete="off" :disabled="editor.couldwrite" :key="index"></el-input></el-col>
              <el-col :span="1">&nbsp</el-col>
              <el-col :span="3"><el-button type="success" :loading="button1loading" @click="searchorder" size="small">搜索订单</el-button></el-col>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
        ...
        <el-table :data="detailData" class="tb-edit" v-loading="detaillistLoading" highlight-current-row @current-change="ChangeCurrentRow">
          <el-table-column :label="detailFieldData.field_desc" width="212" v-for="detailFieldData in detailFieldDatas" v-if="detailFieldData.field_type!='hidden'" :key="detailFieldData.field_desc">
            <template slot-scope="scope">
              ...
              <!-- vv此处自定义子表输入类型 -->
              <div class="ms_input" v-else-if="detailFieldData.couldwrite == 0 && detailFieldData.field_type=='choosemts' &&!refuseChange">
                <el-col :span="15"><el-input size="small" readonly placeholder="请选择" v-model="scope.row[detailFieldData.field_name]" v-bind:value="scope.row[detailFieldData.field_name]" @change="handleEdit(scope.$index, scope.row)" @blur="handleInputBlur(scope.$index, scope.row, detailFieldData.field_name)"></el-input></el-col>
                <el-col :span="1">&nbsp</el-col>
                <el-col :span="8"><el-button size="small" type="success" @click="chooselist(detailFieldData.field_name, detailFieldData.field_desc, scope.row)">选择物料</el-button></el-col>
              </div>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他按钮 -->
        <el-button type="primary" @click.native="confirmPurchase" v-if="editForm.purchase_id && editForm.purchase_approval_state == 0">申请采购</el-button>
        <el-button type="primary" @click.native="confirmPurchaseBuy" v-if="editForm.purchase_id && editForm.payoff_approval_state == 0">申请付款</el-button>
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["采购管理","采购单"];
  let breadUrls = [""];
  let modelName = "采购单";
  let detailModelName = '物料';
  let key_name = 'purchase_id'; // 主键名
  let search_filters = ['purchase_id','order_no']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getPurchaseList'; // 查找获取列表API
  let getDetailAPI = 'stockAction.api?getPurchase'; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?savePurchase'; // 保存API
  let exportAPI = 'stockAction.api?exportPurchaseList'; // 导出xls
  let columns = 2;
  let defaultValues = {accept_state:'0',purchase_approval_state:'0',payoff_approval_state:'0'};
  let enums = {
  ...
  let selectoptions = {
    measure_unit: [{value:'id_name',label: 'material_id'}]
  };
  let popselect = {
    vendors_id:{
      modelname: 'vendors_id',
      getlistAPI: 'vendorsAction.api?getVendorsList',
      key_name:'vendors_id'
    },
    material_id:{
      modelname: 'material_id',
      getlistAPI: 'materialAction.api?getMaterialList',
      key_name:'material_id',
      cbname:'material_id_choose',
      search_filter_list: [
        {field_name:'material_id',field_desc:'物料编号'},
        {field_name:'goods_sku',field_desc:'货号'}
      ]
    }
  }; // 选择框信息
  let rules = {
  ...
  let detailRules = {
    material_id : [{ rule: 'required', message: '未选择物料'}],
    purchase_total_price : [{ rule: 'max', value:'100000000' ,message: '数额超过限度'}]
  };
  /* 模板配置结束 */
  ...
  export default{
    ...
    data(){
      return {
        ...
        /* vv此处自定义数据 */
        button1loading: false, // 搜索订单按钮加载中标记
      };
    },
    methods: {
      // 获取列表数据
      ...
      /* vv此处添加自定义方法 */
      material_id_choose(val){
        this.temp_scoperow.material_name = val.material_name;
        this.temp_scoperow.brand_name = val.brand_name;
        this.temp_scoperow.class_name = val.class_name;
        this.temp_scoperow.goods_parameter_name = val.goods_parameter_name;
        this.temp_scoperow.goods_sku = val.goods_sku;
        this.temp_scoperow.vendors_id = val.default_vendors_code;
        this.selectoptions.measure_unit = [
          {value:val.measure_unit,label: val.measure_unit},
          {value:val.sec_measure_unit,label: val.sec_measure_unit},
          {value:val.thi_measure_unit,label: val.thi_measure_unit}
        ];
        var tempMs = {};
        this.temp_scoperow.measure_unit = '';
        if(val.measure_unit != undefined && val.measure_unit != ""){
          eval("tempMs['" + val.measure_unit + "']='" + val.measure_unit + "'");
          this.temp_scoperow.measure_unit = val.measure_unit;
        }
        if(val.sec_measure_unit != undefined && val.sec_measure_unit != ""){
          eval("tempMs['" + val.sec_measure_unit + "']='" + val.sec_measure_unit + "'");
        }
        if(val.thi_measure_unit != undefined && val.thi_measure_unit != ""){
          eval("tempMs['" + val.thi_measure_unit + "']='" + val.thi_measure_unit + "'");
        }
        eval("this.selectenums.measure_unit['" + val.material_id + "'] = tempMs");
      },
      // 搜索订单
      searchorder(val){
        if(this.editForm.order_no == undefined || this.editForm.order_no == ''){
          return;
        }
        let data = {order_no:this.editForm.order_no};
        this.button1loading = true;
        this.$ajax.postu('orderAction.api?getOrderGoodsList', data).then((res) => {
          if (res.status == "ok") {
            ...
            if(data_arr.length == 0){
              ...
              try{
                ...
                for(var i = 0;i < data_arr.length; ++i){
                  ...
                  for(var j = 0;j < this.detailFieldDatas.length; ++j){
                    var field = this.detailFieldDatas[j];
                    if(this.defaultValues[field.field_name]){
                      eval("newline." + field.field_name + "=" + this.defaultValues[field.field_name]);
                    }else{
                      var type_field = field.field_type;
                  ...
                  newline.material_name = data_i.goods_name;
                  newline.plan_purchase_num = data_i.goods_num;
                  newline.actual_purchase_num = data_i.goods_num;
                  newline.goods_parameter_name = data_i.goods_parameters_name;
                  newline.class_name = data_i.class_name;
                  newline.goods_sku = data_i.goods_sku;
                  newline.brand_name = data_i.brand_name;
                  var editidnow = this.editidnow;
                  // 利用goods_id和parameter_id 匹配现有物料，写入material_id
                  let materialBean = data_i.materialBean;
                  if(materialBean.material_id){
                    newline.material_id = materialBean.material_id;
                    newline.vendors_id = materialBean.default_vendors_code;
                    // 写入指定material_id对应的三级单位
                    var tempMs = {};
                    if(materialBean.measure_unit != undefined && materialBean.measure_unit != ""){
                      eval("tempMs['" + materialBean.measure_unit + "']='" + materialBean.measure_unit + "'");
                      newline.measure_unit = materialBean.measure_unit;
                    }
                    if(materialBean.sec_measure_unit != undefined && materialBean.sec_measure_unit != ""){
                      eval("tempMs['" + materialBean.sec_measure_unit + "']='" + materialBean.sec_measure_unit + "'");
                    }
                    if(materialBean.thi_measure_unit != undefined && materialBean.thi_measure_unit != ""){
                      eval("tempMs['" + materialBean.thi_measure_unit + "']='" + materialBean.thi_measure_unit + "'");
                    }
                    this.selectenums.measure_unit[eval("'" + materialBean.material_id + "'")] = tempMs;
                    // 比较数据
                    if(this.turn_undefined(data_i.goods_name) != this.turn_undefined(materialBean.material_name)){
                      diff_str += '名称不匹配:' + this.turn_undefined(data_i.goods_name) + " / " + this.turn_undefined(materialBean.material_name) + ";";
                    }
                    if(this.turn_undefined(data_i.goods_parameters_name) != this.turn_undefined(materialBean.goods_parameters_name)){
                      diff_str += '规格不匹配:' + this.turn_undefined(data_i.goods_parameters_name) + " / " + this.turn_undefined(materialBean.goods_parameters_name) + ";";
                    }
                    if(this.turn_undefined(data_i.goods_sku) != this.turn_undefined(materialBean.goods_sku)){
                      diff_str += '货号不匹配:' + this.turn_undefined(data_i.goods_sku) + " / " + this.turn_undefined(materialBean.goods_sku) + ";";
                    }
                    if(this.turn_undefined(data_i.class_name) != this.turn_undefined(materialBean.class_name)){
                      diff_str += '分类不匹配:' + this.turn_undefined(data_i.class_name) + " / " + this.turn_undefined(materialBean.class_name) + ";";
                    }
                    if(this.turn_undefined(data_i.brand_name) != this.turn_undefined(materialBean.brand_name)){
                      diff_str += '品牌不匹配:' + this.turn_undefined(data_i.brand_name) + " / " + this.turn_undefined(materialBean.brand_name) + ";";
                    }
                    if(diff_str != ''){
                      this.show_delayed_alert(this,'第 ' + (i + 1) + ' 条数据校验警告',diff_str,'warning',warning_delay)
                      warning_delay += 100;
                    }
                  }else{
                    this.show_delayed_alert(this,'第 ' + (i + 1) + ' 条数据物料匹配失败',"请先创建对应货号及规格的物料",'error',warning_delay)
                    warning_delay += 100;
                  }
                  if(editidnow){
                    eval("newline." + key_name + "=editidnow");
                  }
                  this.detailData.push(newline);
                }
              }catch(er) {
                this.$message({
                  message: '查询失败，请联系管理员！（' + er + "）",
                  type: 'error'
                });
              }
            }
      ...
      turn_undefined(obj){
        return obj?(obj==''?' 无 ':obj):' 无 ';
      },
      show_delayed_alert(the_this,title,message,type,delay){
        !function(){
          setTimeout(function(){
            the_this.$notify({
              title: title,
      ...
      confirmPurchase:function(){
        var purchase_id = this.editForm.purchase_id;
        if(purchase_id == '' || purchase_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?confirmPurchase', {purchase_id:purchase_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '确认采购成功',
                type: 'success'
              });
              this.editForm.purchase_approval_state = 1;
              this.changeRefuseChange();
            } else {
              this.$message({
                message: '确认采购失败，请重试！（' + res.error + "）",
                type: 'error'
              });
              this.changeRefuseChange();
            }
      ...
      confirmPurchaseBuy:function(){
        ...
        if(purchase_id == '' || purchase_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?confirmPurchaseBuy', {purchase_id:purchase_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '申请付款成功',
                type: 'success'
              });
              this.editForm.payoff_approval_state = 1;
              this.changeRefuseChange();
            } else {
              this.$message({
                message: '申请付款失败，请重试！（' + res.error + "）",
                type: 'error'
              });
              this.changeRefuseChange();
            }
      ...
      changeRefuseChange(){
        if(this.editForm.payoff_approval_state == 1 || this.editForm.purchase_approval_state == 1){
          this.refuseChange = true;
        }else{
          this.refuseChange = false;
        }
