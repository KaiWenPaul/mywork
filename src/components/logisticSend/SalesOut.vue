  ...
  <el-row class="warp">
    ...
    <el-table :data="modelDatas" highlight-current-row v-loading="listLoading" style="width: 100%;" >
      ...
      <el-table-column label="操作" width="190" fixed="right">
        <template slot-scope="scope">
          <!-- vv自定义操作按钮 -->
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row, 1)" v-if="scope.row.record_state == 1">查看</el-button>
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row)" v-if="scope.row.record_state == 0">编辑</el-button>
          <el-button type="danger" @click="delBook(scope.$index, scope.row)" size="small" v-if="scope.row.record_state == 0">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
        ...
        <el-table :data="detailData" class="tb-edit" v-loading="detaillistLoading" highlight-current-row @current-change="ChangeCurrentRow">
          <el-table-column :label="detailFieldData.field_desc" width="212" v-for="detailFieldData in detailFieldDatas" v-if="detailFieldData.field_type!='hidden'" :key="detailFieldData.field_desc">
            <template slot-scope="scope">
              ...
              <!-- vv此处自定义子表输入类型 -->
              <el-select class="ms_input" size="small" v-model="scope.row[detailFieldData.field_name]" @change="handleInputBlur(scope.$index, scope.row, detailFieldData.field_name)" v-else-if="detailFieldData.couldwrite == 0 && detailFieldData.field_type=='choosetaxrate' &&!refuseChange">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他详情按钮 -->
        <el-button type="primary" @click.native="sendOut" v-if="editForm.record_id && editForm.record_state == 0">确认出库</el-button>
        <el-button type="primary" @click.native="financial" v-if="editForm.record_id && editForm.financial_processing_state == 0">财务处理</el-button>
        <el-button type="primary" @click.native="pay_f" v-if="editForm.record_id && editForm.payment_state == 0">确认回款</el-button>
        <el-button type="primary" @click.native="editSubmit" v-if="editForm.record_state == 0 && editForm.financial_processing_state == 0 && editForm.payment_state == 0">保存</el-button>
      </div>
    </el-dialog>
    <!-- 导出弹框 -->
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["物流发货","销售出库"];
  let breadUrls = [""];
  let modelName = "销售出库";
  let detailModelName = '出库记录';
  let key_name = 'record_id'; // 主键名
  let search_filters = ['record_id',"customer_id",'order_no','logistics_company_name']; // 直接搜索键名
  let search_filters_other = [
    {name:"material_id",type:"chooselist",desc:"物料编号"},
    {name:"start_time",type:"date",desc:"起始出库时间"},
    {name:"end_time",type:"date",desc:"截止出库时间"}
  ]; // 自定义搜索条件
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getOutboundList'; // 查找获取列表API
  let getDetailAPI = 'stockAction.api?getOutbound'; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?saveOutbound'; // 保存API
  let exportAPI = 'stockAction.api?exportOutboundList'; // 导出xls
  let columns = 3;
  let defaultValues = {
    record_type:'sales_outbound', record_state:'0', payment_state:"0",tax_rate:'17',financial_processing_state:"0",rate:"1",after_amount:"0",record_time:(new Date()).format('yyyy-MM-dd hh:mm:ss'),account_id:window.sessionStorage.getItem('account_id'),account_name:window.sessionStorage.getItem('account_name')
  }; // 默认值,形如state:'0',time:new Date()
  let enums = {
  ...
  let popselect = {
    ...
    stock_id:{
      modelname: 'stock_id',
      getlistAPI: 'stockAction.api?getStockList',
      key_name:'stock_id',
      cbname:'stock_id_choose',
      search_filter_list: [
        {field_name:'stock_id',field_desc:'库存编号'},
        {field_name:'material_id',field_desc:'物料编号'}
      ],
      detail_limit_searches: ['material_id']
    },
    storehouse_id:{
      modelname: 'storehouse_id',
      getlistAPI: 'stockAction.api?getStorehouseList',
      key_name:'storehouse_id',
      cbname:'storehouse_id_choose'
    },
    customer_id:{
      modelname: 'customer_id',
      getlistAPI: 'customerAction.api?getCustomerList',
      key_name:'customer_id',
      cbname:'customer_id_choose',
      search_filter_list: [
        {field_name:'customer_name',field_desc:'客户名称'}
      ]
    }
  }; // 选择框信息
  let rules = {
    record_time: [
      { type: 'date', required: true, message: '出库时间必填', trigger: 'blur' }
    ],
    customer_id: [
  ...
  let detailRules = {
    stock_id : [{ rule: 'required', message: '未填写库存编号'}]
  };
  /* 模板配置结束 */
  ...
  export default{
    ...
    methods: {
      ...
      getAllModels(){
        // hy add 只取指定的出库类型
        this.filters.record_type  = "sales_outbound";
      ...
      /* vv此处添加自定义方法 */
      stock_id_choose(val){
        this.temp_scoperow.storehouse_id = val.storehouse_id;
        this.temp_scoperow.place_id = val.place_id;
        this.temp_scoperow.material_id = val.material_id;
      ...
      storehouse_id_choose(val){
        this.temp_scoperow.storehouse_name = val.storehouse_name;
      },
      customer_id_choose(val){
        this.editForm.customer_name = val.customer_name;
        try{
          if(val.pay_period != '' && val.pay_period != undefined && this.editForm.record_time != undefined && this.editForm.record_time != ''){
            var date_toc = new Date(this.editForm.record_time);
            date_toc = date_toc.setDate(this.editForm.record_time.getDate() + 30);
            this.editForm.repayments_the_term = new Date(date_toc).format('yyyy-MM-dd hh:mm:ss');
          }else {
            this.editForm.repayments_the_term = '';
          }
        }catch (e) {
          console.log(e)
        }
      ...
      // 计算总金额
      check_sum_after_amount(){
        var sum_after_amount = 0;
        for(var i = 0;i<this.detailData.length; ++i){
          var detailData_i = this.detailData[i];
          let sum_row = acc.mul(detailData_i.after_amount,detailData_i.outbound_num);
          sum_after_amount = acc.add(sum_after_amount, sum_row);
        }
        this.editForm.total_stock_money = sum_after_amount;
      },
      // 计算总数量
      check_sum_num(){
        var sum_outbound_num = 0;
        for(var i = 0;i<this.detailData.length; ++i){
          var detailData_i = this.detailData[i];
          sum_outbound_num = acc.add(sum_outbound_num, detailData_i.outbound_num);
        }
        this.editForm.total_stock_num = sum_outbound_num;
      },
      row_change(index, row, field_name){
        if ( field_name == "stock_price"){
          this.temp_scoperow.after_amount = acc.mul(row.stock_price,row.rate);
          this.check_sum_after_amount();
          let c_rate = acc.div(row.tax_rate, 100);
          this.temp_scoperow.tax = acc.mul(row.after_amount,c_rate);
          this.temp_scoperow.after_amount_no_tax = acc.sub(row.stock_price, row.tax);
        }else if (field_name == 'outbound_num'){
          this.check_sum_after_amount();
          this.check_sum_num();
        }else if(field_name == "tax_rate"){
          let c_rate = acc.div(row.tax_rate, 100);
          this.temp_scoperow.tax = acc.mul(row.stock_price,c_rate);
          this.temp_scoperow.after_amount_no_tax = acc.sub(row.stock_price, row.tax);
        }else if(field_name == 'rate'){
          this.temp_scoperow.after_amount = acc.mul(row.stock_price,row.rate);
          this.check_sum_after_amount();
          let c_rate = acc.div(row.tax_rate, 100);
          this.temp_scoperow.tax = acc.mul(this.temp_scoperow.stock_price,c_rate);
          this.temp_scoperow.after_amount_no_tax = acc.sub(row.stock_price, row.tax);
        }
      },
      sendOut(){
        var record_id = this.editForm.record_id;
        if(record_id == '' || record_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?updatePaymentState', {record_id:record_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '确认出库成功',
                type: 'success'
              });
              this.editForm.record_state = 1;
              this.refuseChange = true;
            } else {
              this.$message({
                message: '确认出库失败，请重试！（' + res.error + "）",
                type: 'error'
              });
              this.changeRefuseChange();
            }
          ...
          if (res.status == "ok") {
            ...
            if(data_arr.length == 0){
              ...
              for(var i = 0;i < data_arr.length; ++i){
                ...
                newline.stock_price = data_i.goods_price;
                newline.outbound_num = data_i.goods_num;
                newline.after_amount = data_i.total_price;
                let c_rate = acc.div(newline.tax_rate, 100);
                newline.tax = acc.mul(newline.stock_price,c_rate);
                newline.after_amount_no_tax = acc.sub(newline.stock_price, newline.tax);
                ...
                if(materialBean.material_id) {
                  ...
                  newline.material_name = materialBean.material_name;
      ...
      financial(val){
        var record_id = this.editForm.record_id;
        this.$ajax.postu('stockAction.api?updateFinancialProcessing', {record_id:record_id}).then((res) => {
          if (res.status == "ok") {
            this.$message({
              message: '已确认处理',
              type: 'success'
            });
            this.editForm.financial_processing_state = 1;
            this.changeRefuseChange();
          } else {
            this.$message({
              message: '确认处理失败，请重试！（' + res.error + "）",
              type: 'error'
            });
            this.changeRefuseChange();
          }
      ...
      pay_f(val){
        var record_id = this.editForm.record_id;
        this.$ajax.postu('stockAction.api?updatePaymentSuccess', {record_id:record_id}).then((res) => {
          if (res.status == "ok") {
            this.$message({
              message: '已确认完成回款',
              type: 'success'
            });
            this.editForm.payment_state = 1;
            this.changeRefuseChange();
          } else {
            this.$message({
              message: '确认完成回款失败，请重试！（' + res.error + "）",
              type: 'error'
            });
            this.changeRefuseChange();
          }
      ...
      changeRefuseChange(){
        if(this.editForm.financial_processing_state > 0 || this.editForm.payment_state > 0 || this.editForm.record_state > 0){
          this.refuseChange = true;
        }else {
          this.refuseChange = false;
        }
