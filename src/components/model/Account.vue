  ...
  <el-row class="warp">
    ...
    <el-table :data="modelDatas" highlight-current-row v-loading="listLoading" style="width: 100%;" >
      ...
      <el-table-column label="操作" width="240" fixed="right">
        <template slot-scope="scope">
          <!-- vv自定义按钮 -->
          <el-button size="small" type="warning">修改密码</el-button>
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["系统管理","账号管理"];
  let breadUrls = [""];
  let modelName = "账号";
  let detailModelName = '';
  let key_name = 'account_id'; // 主键名
  ...
  let getListAPI = 'systemAction.api?getAccountList'; // 查找获取列表API
  let getDetailAPI = ''; // 获取编辑页子表信息API
  let saveAPI = 'systemAction.api?saveAccount'; // 保存API
  let columns = 2;
  let defaultValues = {};
  ...
  let popselect = {
    ...
    role_id:{
      modelname: 'role_id',
      getlistAPI:'systemAction.api?getRoleList',
      key_name:'role_id',
    }
  }; // 选择框信息
  let rules = {
    ...
    account_nickname: [
      ...
      { min: 2, max: 250, message: '长度不符合要求', trigger: 'blur' }
    ],
    merchants_id: [
      {required: true, message: '请选择所属商户编号', trigger: 'blur'}
    ],
    role_id: [
      {required: true, message: '请选择所属角色编号', trigger: 'blur'}
    ],
    contact_email:[
      { validator: (rule, value, callback) => {
        if (value){
          if (value.search(/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/) == -1 && value.length > 0){
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            if (this.detailData.length == 0 && edit_type == "details"){
              ...
              // hy add 解决account_name的冲突问题
              pros.name = this.editForm.account_name;
