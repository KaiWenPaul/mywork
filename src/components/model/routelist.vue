      ...
      <el-table :data="modelDatas" highlight-current-row v-loading="listLoading"
        ...
        <el-table-column label="操作" width="30%">
          <template slot-scope="scope">
            <el-button size="small" type="info"><router-link :to="{path:'/route_childlist',query:{parent_id:scope.row.route_id}}"><span style="color:#fff;">子分类</span></router-link></el-button>
...
<script>
  ...
  export default{
    data(){
      return {
        ...
        jsonDatas:{//编辑和新增的时候用
          ...
          menuShow:'true',
          leaf:'false',
          iconCls:'iconfont icon-zhangdan',
          parent_id:'-1'
        },
        //编辑相关数据
    ...
    methods: {
     getAllModels(){
          var data = {parent_id:"-1",account_name:this.account_name,account_token:this.account_token};
          this.$ajax.post('/systemAction.api?getRouteListByParentId',data).then((res)=>{
                if(res.status=="ok"){
      ...
      showEditDialog: function (index, row) {
        ...
        this.editForm = Object.assign(this.jsonDatas,{route_id:row.route_id,name:row.name,path:row.path,sort:row.sort});
      },
      //编辑
      editSubmit: function () {
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              let para = Object.assign({action_token:'u'}, this.editForm);
