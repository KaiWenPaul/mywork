...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["库存管理","出入库汇总"];
  let breadUrls = [""];
  let modelName = "出入库汇总";
  let detailModelName = '汇总详情';
  let key_name = ''; // 主键名
  let search_filters = []; // 直接搜索键名
  let search_filters_other = [
    {name:"stock_id",type:"list",desc:"库存编号"},
    {name:"material_id",type:"list",desc:"物料编号"},
    {name:"material_name",type:"list",desc:"物料名称"},
    {name:"goods_sku",type:"list",desc:"货号"},
    {name:"batch_no",type:"list",desc:"进货批次"},
    {name:"start_time",type:"date",desc:"起始搜索时间"},
    {name:"end_time",type:"date",desc:"截止搜索时间"}
  ]; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "normal"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getInboundOutboundList'; // 查找获取列表API
