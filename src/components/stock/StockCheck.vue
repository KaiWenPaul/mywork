  ...
  <el-row class="warp">
    ...
    <el-table :data="modelDatas" highlight-current-row v-loading="listLoading" style="width: 100%;" >
      ...
      <el-table-column label="操作" width="190" fixed="right">
        <template slot-scope="scope">
          <!-- vv自定义操作按钮 -->
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row, 1)" v-if="scope.row.inventory_state == 1">查看</el-button>
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row)" v-if="scope.row.inventory_state == 0">编辑</el-button>
          <el-button type="danger" @click="delBook(scope.$index, scope.row)" v-if="scope.row.inventory_state == 0" size="small">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他详情按钮 -->
        <el-button type="primary" @click.native="confirmInventory" v-if="editForm.inventory_id && editForm.inventory_state == 0">确认盘点</el-button>
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["库存管理","库存盘点"];
  let breadUrls = [""];
  let modelName = "库存盘点";
  let detailModelName = '库存盘点';
  let key_name = 'inventory_id'; // 主键名
  let search_filters = ['inventory_id']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getInventoryList'; // 查找获取列表API
  let getDetailAPI = 'stockAction.api?getInventory'; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?saveInventory'; // 保存API
  let columns = 3;
  let defaultValues = {inventory_time:(new Date()).format('yyyy-MM-dd hh:mm:ss'),inventory_state:'0'}; // 默认值
  let enums = {inventory_state:{0:"未确认",1:"已确认"}}; // 枚举,形如merchants_id: {1:'one', 2:'two'}
    ...
    methods: {
      ...
      chooselist(type_name,type_desc,scope=""){
        ...
        if(eval("this.chooseDlgDataInfos." + type_name)){
          ...
          console.log(popselect[type_name])
      ...
      /* vv此处添加自定义方法 */
      stock_id_choose(val){
        this.temp_scoperow.paper_num = val.goods_num;
        this.temp_scoperow.real_num = val.goods_num;
        this.temp_scoperow.paper_amount = val.purchase_price;
        this.temp_scoperow.real_amount = val.purchase_price;
      },
      row_change(index, row, field_name){
        if ( field_name == "real_num" || field_name == "real_amount"){
          if(this.temp_scoperow.real_num > this.temp_scoperow.paper_num){
            // 数量盘盈
            this.temp_scoperow.profit_num = acc.sub(this.temp_scoperow.real_num,this.temp_scoperow.paper_num);
            this.temp_scoperow.losses_num = 0;
          }else{
            // 数量盘亏
            this.temp_scoperow.profit_num = 0;
            this.temp_scoperow.losses_num = acc.sub(this.temp_scoperow.paper_num,this.temp_scoperow.real_num);
          }
          let temp_ls_amount_all = acc.mul(this.temp_scoperow.paper_num, this.temp_scoperow.paper_amount);
          let temp_after_amount_all = acc.mul(this.temp_scoperow.real_num, this.temp_scoperow.real_amount);
          if(temp_after_amount_all >= temp_ls_amount_all){
            // 金额盘盈
            this.temp_scoperow.profit_amount = acc.sub(temp_after_amount_all,temp_ls_amount_all);
            this.temp_scoperow.losses_amount = 0;
          }else{
            // 金额盘亏
            this.temp_scoperow.profit_amount = 0;
            this.temp_scoperow.losses_amount = acc.sub(temp_ls_amount_all,temp_after_amount_all);
          }
        }
      },
      confirmInventory(){
        var inventory_id = this.editForm.inventory_id;
        if(inventory_id == '' || inventory_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?confirmInventory', {inventory_id:inventory_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '确认盘点成功',
                type: 'success'
              });
              this.editForm.inventory_state = 1;
              this.changeRefuseChange();
            } else {
              this.$message({
                message: '确认盘点失败，请重试！（' + res.error + "）",
                type: 'error'
              });
              this.changeRefuseChange();
            }
      ...
      changeRefuseChange(){
        if(this.editForm.inventory_state == 1){
          this.refuseChange = true;
        }else{
          this.refuseChange = false;
        }
