...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["库存管理","库存信息"];
  let breadUrls = [""];
  let modelName = "库存";
  let detailModelName = '';
  let key_name = 'stock_id'; // 主键名
  let search_filters = ['stock_id','goods_sku','material_id','batch_no']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "normal"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getStockList'; // 查找获取列表API
  let getDetailAPI = ''; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?saveStock'; // 保存API
  let exportAPI = 'stockAction.api?exportStockList'; // 导出xls
  let columns = 3;
  let defaultValues = {storehouse_id:window.sessionStorage.getItem('storehouse_id')}; // 默认值,形如state:'0',time:new Date()
  ...
  export default{
    ...
    methods: {
      ...
      /* vv此处添加自定义方法 */
      material_id_choose:function(val){
        this.editForm.material_name = val.material_name;
        this.editForm.parameter_name = val.goods_parameter_name;
        this.editForm.brand_name = val.brand_name;
        this.editForm.class_name = val.class_name;
        this.editForm.goods_sku = val.goods_sku;
        this.editForm.measure_unit = val.measure_unit; // 选择时默认为基础计量单位
      }
    },
    mounted() {
