...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["库存管理","库存变更记录"];
  let breadUrls = [""];
  let modelName = "库存变更记录";
  let detailModelName = '库存变更记录';
  let key_name = 'stock_record_id'; // 主键名
  let search_filters = ['stock_id']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "normal"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getStockRecordList'; // 查找获取列表API
  ...
  let enums = {
    detailed_type:{
      confirm_inbound:'采购入库',
      refund_inbound:'退货入库',
      others_inbound:'其他入库',
      sales_outbound:'销售出库',
      return_factory:'退货返厂',
      exchange_outbound:'换货出库',
      other_outbound:'其他出库',
      manual_add:'手动增加',
      manual_reduce:'手动减少'
    }
