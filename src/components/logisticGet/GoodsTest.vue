  ...
  <el-row class="warp">
    ...
    <el-table :data="modelDatas" highlight-current-row v-loading="listLoading" style="width: 100%;" >
      ...
      <el-table-column label="操作" width="190" fixed="right">
        <template slot-scope="scope">
          <!-- vv自定义按钮 -->
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row, 1)" v-if="scope.row.delivery_state == 1">查看</el-button>
          <el-button size="small" @click="showEditDialog(scope.$index, scope.row)" v-if="scope.row.delivery_state == 0">编辑</el-button>
          <el-button type="danger" @click="delBook(scope.$index, scope.row)" size="small" v-if="scope.row.delivery_state == 0">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他详情按钮 -->
        <el-button type="primary" @click.native="deliver" v-if="editForm.delivery_check_id && editForm.delivery_state == 0">确认验收</el-button>
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["物流收货","货物验收"];
  let breadUrls = [""];
  let modelName = "货物验收";
  let detailModelName = '验收物料信息';
  let key_name = 'delivery_check_id'; // 主键名
  let search_filters = ['delivery_check_id']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getDeliveryCheckList'; // 查找获取列表API
  let getDetailAPI = 'stockAction.api?getDeliveryCheck'; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?saveDeliveryCheck'; // 保存API
  let exportAPI = 'stockAction.api?exportDeliveryCheckList'; // 导出xls
  let columns = 3;
  let defaultValues = {delivery_state:'0',delivery_type:'0',delivery_time:(new Date()).format('yyyy-MM-dd hh:mm:ss'),
    account_id:window.sessionStorage.getItem('account_id'),
    storehouse_id:window.sessionStorage.getItem('storehouse_id'),
    storehouse_name:window.sessionStorage.getItem('storehouse_name')
  }; // 默认值
  let enums = {
    delivery_state:{0:'未收货', 1:'已收货'}
  }; // 枚举,形如merchants_id: {1:'one', 2:'two'}
  let selectoptions = {
    delivery_type: [
      {value:'0',label: '物流公司送货'},
      {value:'1',label: '供应商直接送货'},
      {value:'2',label: '自提'}
    ],
    measure_unit: [{value:'id_name',label: 'material_id'}]
  }; // 选项,主表形如is_devi: [ {value:'0',label: '未交付'}, {value:'1',label: '已交付'} ]  物流公司送货/供应商直接送货/自提，子表写一个对应的外键名
  let popselect = {
    ...
    purchase_id:{
      modelname: 'purchase_id',
      getlistAPI: 'stockAction.api?getPurchaseList',
      key_name:'purchase_id',
      enums:{
        accept_state:{0:'未验收', 1:'已验收'},
        purchase_approval_state:{0:'未审批', 1:'审批通过', 2:'审批不通过'},
        payoff_approval_state:{0:'未审批', 1:'审批通过', 2:'审批不通过'}
      },
      cbname:'purchase_id_choose',
      search_filter_list: [
        {field_name:'purchase_id',field_desc:'采购单编号'}
      ],
      default_select:[
        {field_name:'accept_state',field_value:'0'},
        {field_name:'purchase_approval_state',field_value:'1'},
        {field_name:'payoff_approval_state',field_value:'1'}
      ]
    }
  }; // 选择框信息，样式见MaterialManage.vue
  let rules = {
    storehouse_id: [
      { required: true, message: '请选择仓库编号', trigger: 'blur'},
    ]}; // 保存约束
  let detailRules = {
    material_id:[{ rule: 'required', message: '未填写物料编号'}]
  };
  /* 模板配置结束 */
  ...
  export default{
    ...
    methods: {
      ...
      /* vv此处添加自定义方法 */
      my_choose(val){
        this.editForm.storehouse_name = val.storehouse_name;
      },
      ...
      // 获取采购单子表并填充
      purchase_id_choose(val){
        this.$ajax.postu('stockAction.api?getPurchase', {purchase_id: val.purchase_id}).then((res) => {
          if (res.status == "ok") {
            var detaildatas = res.data;
            for (var detaildata in detaildatas) {
              if (/.*Beans$/.exec(detaildata) != undefined) {
                ...
                for(var j = 0;j < data_arr.length; ++j){
                  ...
                  newline.purchase_material_id = data_i.purchase_material_id;
                  ...
                  newline.material_num = data_i.actual_purchase_num;
      ...
      deliver:function(){
        var record_id = this.editForm.delivery_check_id;
        if(record_id == '' || record_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?confirmDeliveryCheck', {delivery_check_id:record_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '确认验收成功',
                type: 'success'
              ...
              this.refuseChange = true;
              this.editForm.delivery_state = 1;
            } else {
              this.$message({
                message: '确认验收失败，请重试！（' + res.error + "）",
                type: 'error'
              });
            }
            this.chooseDlglistLoading = false;
