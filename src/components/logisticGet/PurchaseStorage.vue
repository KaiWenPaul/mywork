  ...
  <el-row class="warp">
    ...
    <el-dialog :title="edititle" size="large" v-model="editFormVisible" :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <!-- vv其他详情按钮 -->
        <el-button type="primary" @click.native="importing" v-if="editForm.record_id && editForm.record_state == 0">确认入库</el-button>
        <el-button type="primary" @click.native="payoff_f" v-if="editForm.record_id && editForm.is_payoff == 0">确认付款</el-button>
        <el-button type="primary" @click.native="editSubmit" v-if="editForm.record_state == 0">保存</el-button>
      </div>
    </el-dialog>
    <!-- 导出弹框 -->
...
<script>
  ...
  /* vv模板配置 */
  let breadNames = ["物流收货","采购入库"];
  let breadUrls = [""];
  let modelName = "入库";
  let detailModelName = '入库记录';
  let key_name = 'record_id'; // 主键名
  let search_filters = ['record_id']; // 直接搜索键名
  let search_filters_other = []; // 自定义搜索条件,形如{name:"start_time",type:"date",desc:"起始出库时间"}
  let edit_type = "details"; // 单表结构："normal" / 主子表结构："details"
  let getListAPI = 'stockAction.api?getInboundList'; // 查找获取列表API
  let getDetailAPI = 'stockAction.api?getInbound'; // 获取编辑页子表信息API
  let saveAPI = 'stockAction.api?saveInbound'; // 保存API
  let exportAPI = 'stockAction.api?exportInboundList'; // 导出xls
  let columns = 3;
  let defaultValues = {record_state:'0',record_type:'confirm_inbound',tax_rate:'17',is_payoff:'0',record_time:(new Date()).format('yyyy-MM-dd hh:mm:ss'),
    account_id:window.sessionStorage.getItem('account_id'),
    account_name:window.sessionStorage.getItem('account_name'),
    storehouse_id:window.sessionStorage.getItem('storehouse_id')
  ...
  }; // 选项,形如is_devi: [ {value:'0',label: '未交付'}, {value:'1',label: '已交付'} ]
  let popselect = {
    vendors_id:{ // 选择框字段名
      modelname: 'vendors_id', // 选择框字段名
      getlistAPI: 'vendorsAction.api?getVendorsList', // 选择框获取列表接口
      key_name:'vendors_id', // 选择框列表主键（默认回带字段）
      cbname:'my_choose'  // 若有则为自定义回调函数名，参数：选中项，在选择行后执行
    },
    storehouse_id:{ // 选择框字段名
      modelname: 'storehouse_id', // 选择框字段名
      getlistAPI: 'stockAction.api?getStorehouseList', // 选择框获取列表接口
      key_name:'storehouse_id', // 选择框列表主键（默认回带字段）
      cbname:'storehouse_id_choose'  // 若有则为自定义回调函数名，参数：选中项，在选择行后执行
    },
    material_id:{
    ...
    delivery_check_id:{
      modelname: 'delivery_check_id',
      getlistAPI: 'stockAction.api?getDeliveryCheckList',
      key_name:'delivery_check_id',
      cbname:'delivery_check_id_choose',
      search_filter_list: [
        {field_name:'delivery_check_id',field_desc:'验收单编号'}
      ],
      enums:{
        delivery_state:{0:'未验收', 1:'已验收'},
      },
      default_select:[
        {field_name:'delivery_state',field_value:'1'},
      ]
    }
  }; // 选择框信息
  let rules = {
  ...
  let detailRules = {
    ...
    storehouse_id : [{ rule: 'required', message: '未选择仓库'}],
    purchase_price : [{ rule: 'required', message: '未填写采购价格'},{ rule: 'max', value:'100000000' ,message: '数额超过限度'}],
    tax : [{ rule: 'max', value:'100000000' ,message: '数额超过限度'}]
  };
  /* 模板配置结束 */
  ...
  export default{
    ...
    methods: {
      ...
      getAllModels(){
        // hy add 只取指定的入库类型
        this.filters.record_type  = "confirm_inbound";
                  ...
                  for(var key in this.editFormDetailRules){
                    ...
                    for(var ls = 0;ls < check_arr.length ; ++ls){
                      ...
                      if(check_info.rule == 'required'){
                        ...
                        if(check_fs){
                          console.log('err')
                          console.log(check_fs)
                          console.log(check_info)
                          console.log(check_info.value)
      ...
      /* vv此处添加自定义方法 */
      my_choose(val){
        this.editForm.vendors_name = val.vendors_name;
      },
      ...
      delivery_check_id_choose(val){
        this.button1loading = true;
        this.$ajax.postu('stockAction.api?getDeliveryCheck', {delivery_check_id: val.delivery_check_id}).then((res) => {
          if (res.status == "ok") {
            var detaildatas = res.data;
            var this_storehouse_id = detaildatas['storehouse_id'];
            for (var detaildata in detaildatas) {
              if (/.*Beans$/.exec(detaildata) != undefined) {
                var data_arr = detaildatas[detaildata];
                for(var j = 0;j < data_arr.length; ++j){
                  var data_i = data_arr[j];
                  var newline = {
                    action_token:"c"
                  };
                  for(var i = 0;i < this.detailFieldDatas.length; ++i){
                  ...
                  newline.material_id = data_i.material_id;
                  newline.material_name = data_i.material_name;
                  newline.goods_sku = data_i.goods_sku;
                  newline.goods_parameter_name = data_i.goods_parameter_name;
                  newline.brand_name = data_i.brand_name;
                  newline.class_name = data_i.class_name;
                  newline.measure_unit = data_i.measure_unit;
                  newline.material_num = data_i.material_num;
                  newline.storehouse_id = this_storehouse_id;
      ...
      importing:function(){
        ...
        if(record_id == '' || record_id == undefined){
          ...
          this.$ajax.postu('stockAction.api?confirmInbound', {record_id:record_id}).then((res) => {
            if (res.status == "ok") {
              this.$message({
                message: '确认入库成功',
                type: 'success'
      ...
      row_change:function(index, row, field_name){
        if (field_name == 'material_num' || field_name == "purchase_price"){
          var sum_after_amount = 0;
          var sum_count = 0
          for(var i = 0;i<this.detailData.length; ++i){
            var detailData_i = this.detailData[i];
            sum_after_amount = acc.add(sum_after_amount, acc.mul(detailData_i.material_num,detailData_i.purchase_price));
            sum_count = acc.add(sum_count,detailData_i.material_num);
          }
          this.editForm.total_inventory_money = sum_after_amount;
          this.editForm.total_inventory_num = sum_count;
        }
        if(field_name == "purchase_price" || field_name == "tax_rate"){
          let c_rate = acc.div(row.tax_rate, 100);
          this.temp_scoperow.tax = acc.mul(row.purchase_price,c_rate);
          this.temp_scoperow.purchase_price_no_tax = acc.sub(row.purchase_price, row.tax);
        }
      },
      // 确认付款
      payoff_f:function(){
